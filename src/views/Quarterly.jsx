...
export const Quarterly = ({ translate }) => {
    ...
    const [twoYearMonth, setTwoYearMonth] = useState(utilsService.getLast24Months())
    ...
    const getDepartmentSum = (departmentId, idx, oneMonth) => {
        ...
        if (!idx && idx !== 0) {
            const sum = newSum.reduce((total, subArray, idx) => {
                if (idx >= twoYearMonth.length) return total
        ...
        <section className="quarterly-section">
            ...
            <ul className="">
                {departments.map((department, departmentIdx) => {
                    return ((itemsDisplay && getDepartmentSum(department['bar-code']) !== 0) || (!itemsDisplay && getDepartmentSum(department.id) !== 0) || departmentIdx === 0) && <li key={department.id || department._id} className="flex align-center space-between">
                            ...
                            {twoYearMonth.map((monthGroup, idx) =>
                                <div key={monthGroup[0].year + monthGroup[0].number} className="flex column">
                                    {departmentIdx === 0 && <p className="bold text-center">
                                        {monthGroup[0].number}.{monthGroup[0].year % 100} - {monthGroup[2].number}.{monthGroup[2].year % 100}
                                    </p>}
                                    {(showPopup === (monthGroup[0].year + '' + idx + (itemsDisplay ? department['bar-code'] : department.id))) && <div className="popup">
                                        <p className="text-center bold">{department.name}</p>
                                        <button className="close-popup" onClick={closePopoup}>X</button>
                                    ...
                                    <p className="text-center pointer" onClick={() => openPopoup((monthGroup[0].year + '' + idx + (itemsDisplay ? department['bar-code'] : department.id)))}>{getDepartmentSum((itemsDisplay ? department['bar-code'] : department.id), idx) || '-'}</p>
                                </div>
                            )}
                        </div>
                    </li>
