...
export const AssemblageDemo = () => {
    ...
    return (
        <section className="assemblage-container">
            ...
            {/* {!showGroups && <input type="text" name="group" value={input1} onChange={onChangeFilter} className="input" placeholder={viewMode === 'items' ? "find items group" : "find  suppliers group"} />} */}
            {!showGroups && viewMode === 'items' && <input type="text" name="group" value={input1} onChange={onChangeFilter} className="input" placeholder={viewMode === 'items' ? "find items group" : "find  suppliers group"} />}
            ...
            {(supplierItems || showGroups) && <button onClick={backToGroups}>Back to groups</button>}
            <ul className="items-list">
                {!showGroups && viewMode === 'items' ? groups.map((group) => {
                    return groupSum((group.id || group._id)) &&
                        <li key={group._id || group.id}>
                            <div className="supplier-container">
                                <button onClick={() => openGroup(group)}>{group.name}</button>
                        ...
                        return (supplierItems ? supplierSum(item._id, item.supplier._id) : supplierSum(item['bar-code'] || item._id)) && <li key={item._id}>
                            <div className="supplier-container">
                                {(supplierItems || viewMode === 'items') ?
                                    ...
                                    : <button onClick={() => getSupplierItems(item._id)}>{item.name}</button>
                                }
