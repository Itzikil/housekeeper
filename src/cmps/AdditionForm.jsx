...
export const AdditionForm = ({ order }) => {
    ...
    const handleChange = ({ target }, idx) => {
        ...
        if (idx || idx === 0) {
            ...
            setItemsLine(lines);
            let linePrice = itemsLine[idx]['price']
            let lineQuantity = itemsLine[idx]['quantity']
            let lineTotal = itemsLine[idx]['total']
            ...
            if (field === '_id' && form.supplier._id && !itemsLine[idx]['price']) {
                ...
                itemsLine[idx]['total'] = item.total
                itemsLine[idx]['discount'] = item.discount
            }

            if (value !== '' && value !== 0) {
                if (linePrice && lineQuantity && lineTotal && field === 'total') {
                    itemsLine[idx]['price'] = checkIfFloat(lineTotal / lineQuantity)
                } else if (linePrice && lineQuantity) {
                    itemsLine[idx]['total'] = checkIfFloat(linePrice * lineQuantity)
                } else if (lineTotal && lineQuantity) {
                    itemsLine[idx]['price'] = checkIfFloat(lineTotal / lineQuantity)
                } else if (lineTotal && linePrice) {
                    itemsLine[idx]['quantity'] = checkIfFloat(lineTotal / linePrice)
                }
            }
        }
        ...
        <div className="additionForm">
            ...
            <form onSubmit={onSaveForm} >
                <div className="input-container">
                    <SearchBar suggestions={supplierService.loadSuppliers()} handleChange={handleChange} name="supplier" value={{ name: form.supplier?.name, _id: form.supplier?._id }} placeholder={'item'}/>
                ...
                <div className="line-container flex column gap20">
                    {itemsLine?.map((item, idx) =>
                        <div className="flex gap10 relative" key={idx + (item._id || idx + 1)}>
                            ...
                            {/* <p>₪{(item.total - (item.discount || 0)).toFixed(2)}</p> */}
                            <input type="number" onChange={(ev) => handleChange(ev, idx)} value={item.total} name="total" placeholder="Total" className="input" />
                ...
                <div className="flex gap20 align-center justify-end">
